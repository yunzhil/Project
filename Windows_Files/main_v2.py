...
#  directory: C:\Users\kiddb\AppData\Local\Programs\Python\Python312\Lib\site-packages\
import pandas as pd 
import numpy as np 
import quaternion
import ipdb
...
from util import va, synchronization, common, constant_common
from util.mt import constant_mt, preprocessing_mt, calibration_mt, ik_mt
from util.mocap import constant_mocap, preprocessing_mocap, ik_mocap


# Set up the configurations here
f_list = ['MAD', 'MAH', 'EKF', 'VQF', 'Xsens']
f_type = 'Xsens'
dim    = '9D' # only 9D at the moment

if f_type == f_list[0]:
    f_params = [0.1]
elif f_type == f_list[1]:
    f_params = [0.4, 0.3]
elif f_type == f_list[2]:
    f_params = [0.9, 0.9, 0.9]
elif f_type == f_list[3]:
    f_params = [2, 10]
else:
    f_params = None
...
sensor_config  = {'pelvis': 'PELVIS', 
                  'foot_r': 'FOOT_R', 'shank_r': 'SHANK_R_' + selected_setup.upper(), 'thigh_r': 'THIGH_R_' + selected_setup.upper(), 'mfoot_r': 'mFOOT_R', 'mthigh_r': 'mTHIGH_R', 'mshank_r': 'mSHANK_R',
...
#  Meta Data that can be added in later
# --- Compare IMU vs. mocap --- #
meta_fn = constant_common.IN_LAB_PATH + 'eval_overground_walking_001.xlsx'
meta_dt = pd.read_excel(meta_fn, sheet_name = 's' + str(subject))
meta_dt.columns = pd.RangeIndex(meta_dt.columns.size)
ipdb.set_trace()
start_col = 0
stop_col = 0
## what rows and columns do we want in the excel sheet??
##start_ja_id = int(meta_dt[start_col][constant_common.MAPPING_TASK_TO_ID[selected_task]])
##stop_ja_id  = int(meta_dt[stop_col][constant_common.MAPPING_TASK_TO_ID[selected_task]])
start_ja_id = 0
stop_ja_id = 1
