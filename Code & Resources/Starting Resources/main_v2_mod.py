...
selected_task  = 'treadmill_walking'
subject        = 3
...
def update(i):
    ...
    x_pelvis, y_pelvis, z_pelvis    = va.rotate_frame(np.matmul(quaternion.as_rotation_matrix(main_orientation_mt['pelvis'][i]), seg2sens['pelvis'].T), x_axis, y_axis, z_axis)
    x_thigh_r, y_thigh_r, z_thigh_r = va.rotate_frame(np.matmul(quaternion.as_rotation_matrix(main_orientation_mt['thigh_r'][i]), seg2sens['thigh_r'].T), x_axis, y_axis, z_axis)
    x_shank_r, y_shank_r, z_shank_r = va.rotate_frame(np.matmul(quaternion.as_rotation_matrix(main_orientation_mt['shank_r'][i]), seg2sens['shank_r'].T), x_axis, y_axis, z_axis)
    x_foot_r, y_foot_r, z_foot_r    = va.rotate_frame(np.matmul(quaternion.as_rotation_matrix(main_orientation_mt['foot_r'][i]), seg2sens['foot_r'].T), x_axis, y_axis, z_axis)
    x_thigh_l, y_thigh_l, z_thigh_l = va.rotate_frame(np.matmul(quaternion.as_rotation_matrix(main_orientation_mt['thigh_l'][i]), seg2sens['thigh_l'].T), x_axis, y_axis, z_axis)
    x_shank_l, y_shank_l, z_shank_l = va.rotate_frame(np.matmul(quaternion.as_rotation_matrix(main_orientation_mt['shank_l'][i]), seg2sens['shank_l'].T), x_axis, y_axis, z_axis)
    x_foot_l, y_foot_l, z_foot_l    = va.rotate_frame(np.matmul(quaternion.as_rotation_matrix(main_orientation_mt['foot_l'][i]), seg2sens['foot_l'].T), x_axis, y_axis, z_axis)
...
anim = animation.FuncAnimation(fig, update, frames = range(1300, 1500))
# anim = animation.FuncAnimation(fig, update)

from matplotlib.animation import PillowWriter
writer = PillowWriter(fps = 100)

anim.save('s' + str(subject) + '_frame_riann_walking' + selected_setup + '.gif', writer = writer)
