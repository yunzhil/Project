...
def get_hc_n_to_mocap(heel_marker_y, toe_marker_y, task, fs = 100, remove = 10, vis = False):
    ...
    temp_hc_index, temp_hc_value = find_peaks(-1*heel_marker_y[remove:], height = [-1, 0], distance = min_peak_distance_hc)
    hc_index, hc_value           = remove_noisy_peaks_mocap(temp_hc_index, -1*temp_hc_value['peak_heights'], task)

    temp_to_index, temp_to_value = find_peaks(-1*toe_marker_y[remove:], height = [-1, 0], distance = min_peak_distance_to)
    ...
    #add the removed index back
    gait_events['hc_index'] += remove
    gait_events['to_index'] += remove
